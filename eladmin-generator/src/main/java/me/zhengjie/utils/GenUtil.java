...
//        templateNames.add("ServiceImpl");
    ...
    private static Map<String, Object> getGenMap(List<ColumnInfo> columnInfos, GenConfig genConfig) {
        ...
        genMap.put("prefixClassName", className.replace("Entity", ""));
        // 保存小写开头的类名
        ...
        for (ColumnInfo column : columnInfos) {
            ...
            if (!StringUtils.isBlank(column.getQueryType())) {
                ...
                queryType.append("'" + StringUtils.toCamelCase(column.getColumnName()) + "," + column.getQueryType() + "', ");
            }
            Map<String, String> export = new HashMap<>();
            export.put("title", column.getRemark());
            export.put("value", StringUtils.toCamelCase(column.getColumnName()));
        ...
        if (queryTypeString.endsWith(", ")) {
            queryTypeString = queryTypeString.substring(0, queryTypeString.length() - 2);
        }
        genMap.put("queryType", queryTypeString);
        ObjectMapper objectMapper = new ObjectMapper();
        objectMapper.configure(JsonGenerator.Feature.QUOTE_FIELD_NAMES, false);
        genMap.put("exportHeaders", objectMapper.writeValueAsString(exportList));
        return genMap;
    }
